...
if (hasInterface) then {
    ...
    if (!isNil {player getVariable "moneyX"}) then { _fields pushBack ["money", player getVariable "moneyX"] };
    _fields pushBack ["rank", rank player];
    _fields pushBack ["is_undercover", (captive player) && {!(player getVariable ["incapacitated", false])}];
    if (!isNil {player getVariable "isAFK"}) then { _fields pushBack ["is_afk", player getVariable ["isAFK", false]] };
